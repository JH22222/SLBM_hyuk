...
import PollItem5 from './components/PollItem5.vue'
...
<template>
  ...
  <div class="container">
    <div v-if="render">
      ...
      <div class="row justify-content-md-center" v-if="(type === 'form5')">
        <div :class="{'col-7':!isMobile, 'col-12':isMobile}">
          <PollItem5 :item="item" :meta="polls.meta" :isMobile="isMobile" v-for="item in polls.item.slice(0,11)"
                     :key="item['poll_key']"/>
        </div>
        <h3 :class="{'col-7':!isMobile, 'col-12':isMobile}"><span
            class="badge bg-dark badge-edit"><strong><em>{{ polls.meta.status }}</em></strong></span></h3>
        <div :class="{'col-7':!isMobile, 'col-12':isMobile}">
          <PollItem5 :item="item" :meta="polls.meta" :isMobile="isMobile" v-for="item in polls.item.slice(11,18)"
                     :key="item['poll_key']"/>
        </div> 
        <h3 :class="{'col-7':!isMobile, 'col-12':isMobile}"><span
            class="badge bg-dark badge-edit"><strong><em>{{ polls.meta.status2 }}</em></strong></span></h3>
        <div :class="{'col-7':!isMobile, 'col-12':isMobile}">
          <PollItem5 :item="item" :meta="polls.meta" :isMobile="isMobile" v-for="item in polls.item.slice(18,27)"
                     :key="item['poll_key']"/>
        </div>
        <h3 :class="{'col-7':!isMobile, 'col-12':isMobile}"><span
            class="badge bg-dark badge-edit"><strong><em>{{ polls.meta.status3 }}</em></strong></span></h3>
        <div :class="{'col-7':!isMobile, 'col-12':isMobile}">
          <PollItem5 :item="item" :meta="polls.meta" :isMobile="isMobile" v-for="item in polls.item.slice(27,65)"
                     :key="item['poll_key']"/>
        </div>
      </div>
  ...
  methods: {
    ...
    parser5(data) {
      ...
      data.forEach((line) => {
        ...
        if (spliter%2 === 0 && spliter < 120 && spliter > 1) {
          meta.selections[count++] = line.split(',');
        }
        if (spliter%2 === 1 && spliter < 120 && spliter > 1) {        
          let temp = line.split(' ');
          const poll_key = temp[0];
          temp.shift();
          const poll = temp.join(' ');
    ...
    next() {
      ...
      if (this.remains.length !== 0) {
        ...
        if (this.dataset.length > this.page_num) {
          ...
          console.log('type : ', self.dataset[this.page_num].type)
        }
