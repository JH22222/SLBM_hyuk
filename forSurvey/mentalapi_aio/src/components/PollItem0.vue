...
<template>
  <div v-if=item.poll_key class="card text-center mb-5"
    ...
    <div class="card-footer text-muted" v-if="item.poll_key !== '3'">
      <!-- {{ poll }}<input class="form-range btn btn-outline-dark" type="range" min="1" max="24" -->
      {{ poll }}<input class="form-range btn btn-outline-dark" type="range" min="1" max="24" step="0.5"
